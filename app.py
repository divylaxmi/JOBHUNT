...
def user_signup_insert():
    if request.method=='POST':
        ...
        # validation required
        frm=[fname,lname,email,mobile,password]
        valid = myvalidate() 
        ...
        op.user_signup_insert(fname,lname,email,mobile,password)

        #------- email verification--------------------------------
        global otp
        otp = random.randint(100000,999999)

        msg = Message('Dzinex Email verification',sender ='dzinextech@outlook.com', recipients = [email])
...
def user_profile_update():
    if 'email' in session:
        if request.method=='POST':
            ...
            mobile=request.form['mobile']
            op = user_operation()  # object create
            op.user_profile_update(fname,lname,mobile)
...
def company_job_post_insert():
    if 'company_email' in session:
        if request.method=='POST':
            ...
            elegibility=request.form['elegibility']
            ...
            # validation
            frm=[job_name,no_of_post,elegibility,exp,apply_date,descp,photo_obj]
            valid = myvalidate() 
            v=valid.required(frm)
            if(v==False):
                flash("field must be filled!!")
            ...
            op = company_operation()  # object create
            op.company_job_post_insert(job_name,no_of_post,elegibility,qualification,exp,apply_date,descp,photo)
...
def company_job_delete():
    if 'company_email' in session:
        ...
        flash("Job is deleted successfully!!")
        return redirect(url_for('company_job_list'))
    else:
        flash("kindly login to access this page!!")
        return redirect(url_for('company_login'))
