...
class company_operation:
    def connection(self):
        db=mysql.connector.connect(host="localhost", port="3306", user="root", password="root", database="jobhunt")
        return db
    ...
    def company_job_post_insert(self,job_name,no_of_post,elegibility,qualification,exp,apply_date,descp,photo):
        db = self.connection()
        mycursor = db.cursor()
        sq="insert into job(company_email,job_name,no_of_post,elegibility,qualification,exp,apply_date,descp,photo) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        
        record=[session['company_email'],job_name,no_of_post,elegibility,qualification,exp,apply_date,descp,photo]
        mycursor.execute(sq,record)
        db.commit()
        mycursor.close()
        db.close()
    ...
    def company_job_applied_view(self,job_id):
        db = self.connection()
        mycursor = db.cursor()
        sq ="select job_id,fname,lname,email,mobile,applied_date from apply_job a, user u where a.user_email=u.email and job_id=%s"
